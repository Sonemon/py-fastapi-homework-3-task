...
async def register(user_data: UserRegistrationRequestSchema,
...
async def request_password_reset_token(
    ...
    SUCCESS_MESSAGE = "If you are registered, you will receive an email with instructions."
    ...
    if not user or not user.is_active:
        return MessageResponseSchema(message=SUCCESS_MESSAGE)
...
async def refresh_access_token(
    ...
    if refresh_token_record.user_id != user_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Token does not match the user.",
        )
